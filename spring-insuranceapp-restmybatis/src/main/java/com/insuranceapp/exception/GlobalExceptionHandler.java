...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleHttpRequestMethodNotSupported(HttpRequestMethodNotSupportedException ex,
		...
		ApiErrors apiErrors = new ApiErrors(timestamp,status.value(), ex.getMessage(), statusMessage);
		headers.add("Error", "Request method is not Supported");
	...
	protected ResponseEntity<Object> handleMissingPathVariable(MissingPathVariableException ex, HttpHeaders headers,
		...
		HttpStatus statusMessage = HttpStatus.valueOf(status.value()); 
		ApiErrors apiErrors = new ApiErrors(timestamp,status.value(), ex.getMessage(), statusMessage);
		headers.add("Error", "Missing the Path Variable");
		return ResponseEntity.status(status).headers(headers).body(apiErrors);
	}
	...
	public ResponseEntity<Object> handleInsuranceNotFound(InsuranceNotFoundException ex){
		...
		ApiErrors apiErrors = new ApiErrors(timestamp,statusMessage.value(), ex.getMessage(), statusMessage);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Error", "Type MissMatch error");
		return ResponseEntity.status(statusMessage).headers(headers).body(apiErrors);
	}
